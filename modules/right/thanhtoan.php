...
if (isset($_POST['payatm'])){header("Location: modules/payatm");}
if (isset($_POST['dathang'])) {
    ...
    foreach ($_SESSION['cart'] as $id_sp => $count) {
            ...
            while ($dong_sp = mysqli_fetch_array($sp, MYSQLI_ASSOC)) {
                ...
                $soluongspmoi = $soluongspcu - $count;
    ...
    foreach ($_SESSION['cart'] as $id_sp => $count) {
        ...
        while ($dong_sp = mysqli_fetch_array($sp, MYSQLI_ASSOC)) {
            ...
            $soluongspmoi2 = $soluongspcu2 - $count;

            $tongtien = $dong_sp['gia'] * $count;
            $sql2 = "insert into chitiethoadonban(id_hdb,id_sp,soluong,thanhtien) value($id_hdb,$id_sp,$count,$tongtien)";
        ...
        <table id="thanhtoan">
            ...
            if (isset($_SESSION['tenkhach'])) {
                ...
                foreach ($_SESSION['cart'] as $id_sp => $count) {
                    ...
                    $sp = mysqli_query($conn, $sql);
                    while ($dong_sp = mysqli_fetch_array($sp, MYSQLI_ASSOC)) {
                        $tongtien2 = $dong_sp['gia'] * $count;
                        $thanhtien2 += $tongtien2;
                        ?>
                        <tr>
                            <td><?php echo $dong_sp['tensp'] ?></td>
                            <td><img src="<?php echo $dong_sp['hinhanh'] ?>" alt=""
            ...
            <tr>
                ...
                <td style="padding-right: 0px" colspan="4"><span><?php echo number_format($thanhtien2, 0, ',', '.') ?>
                        ₫</span></td>
            </tr>
            <tr>
                <td colspan="5">
                    <input style="margin-left: 315px;width: 120px;height: 45px;border: none;color: #fff;background-color: #337ab7;border-color: #2e6da4;border-radius: 10px;"
                           type="submit" name="dathang" value="ĐẶT HÀNG">
                           <input style="margin-left: 315px;width: 120px;height: 45px;border: none;color: #fff;background-color: #337ab7;border-color: #2e6da4;border-radius: 10px;margin-top: 5px;"
                           type="submit" name="payatm" value="Pay bank">
                </td>
